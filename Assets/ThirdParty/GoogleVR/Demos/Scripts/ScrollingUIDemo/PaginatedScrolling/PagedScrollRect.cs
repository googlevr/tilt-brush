...
public class PagedScrollRect : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler {
  ...
  void Update() {
    ...
    if (!isScrolling && GvrControllerInput.IsTouchingLeft) {
      if (!isTrackingTouches) {
        StartTouchTracking();
      } else {
        Vector2 touchDelta = GvrControllerInput.TouchPosLeft - initialTouchPos;
    ...
    if (isScrolling && GvrControllerInput.IsTouchingLeft) {
      Vector2 touchDelta = GvrControllerInput.TouchPosLeft - previousTouchPos;
    ...
    if (GvrControllerInput.TouchUpLeft) {
      StopScrolling();
      StopTouchTracking();
    }

    if (isTrackingTouches && GvrControllerInput.IsTouchingLeft) {
      TrackTouch();
    }
  }
  ...
  private void StartTouchTracking() {
    ...
    initialTouchPos = GvrControllerInput.TouchPosLeft;
  ...
  private void TrackTouch() {
    ...
    previousTouchPos = GvrControllerInput.TouchPosLeft;
    previousTouchTimestamp = Time.time;
  }
