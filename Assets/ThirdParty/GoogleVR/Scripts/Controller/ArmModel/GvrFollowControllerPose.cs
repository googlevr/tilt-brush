    ...
    private void OnPostControllerInputUpdated()
    {
        ...
        switch (pose)
        {
            case Pose.ControllerFromHead:
                if (controllerID == 0)
                {
                    position = ArmModel.ControllerPositionFromHeadLeft;
                    rotation = ArmModel.ControllerRotationFromHeadLeft;
                }
                else
                {
                    position = ArmModel.ControllerPositionFromHeadRight;
                    rotation = ArmModel.ControllerRotationFromHeadRight;
                }
                //Debug.Log("ControllerID " + controllerID.ToString() + " Position<<>> " + position.ToString());
                break;
        ...
        //Debug.Log("ID: " + controllerID + " -- Position<<>> " + position + "  -- Rotation: " + rotation);
    }
}
