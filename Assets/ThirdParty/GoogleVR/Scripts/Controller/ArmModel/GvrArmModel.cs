...
public class GvrArmModel : GvrBaseArmModel{
  ...
  public override Vector3 ControllerPositionFromHeadLeft {
    get {
      return wristPositionLeft;
    }
  }

    public override Vector3 ControllerPositionFromHeadRight
    {
        get
        {
            return wristPositionRight;
        }
    }

    /// Quaternion to represent the controller's rotation relative to
    /// the user's head position.
    public override Quaternion ControllerRotationFromHeadLeft {
    get {
      return wristRotationLeft;
    }
  }
    public override Quaternion ControllerRotationFromHeadRight
    {
        get
        {
            return wristRotationRight;
        }
    }
  ...
  /// NOTE: This is in meatspace coordinates.
  private Vector3 wristPositionLeft;
    private Vector3 wristPositionRight;

    /// Quaternion to represent the wrist's rotation.
    /// NOTE: This is in meatspace coordinates.
    private Quaternion wristRotationLeft;
    private Quaternion wristRotationRight;
  ...
  private float preferredAlpha = 1f;

  /// Backing variable for the TooltipAlphaValue property.
  private float tooltipAlphaValue = 1f;
  ...
  private static readonly Vector3 POINTER_OFFSET = new Vector3(0.0f, 0.0f, 0.01f);
  ...
  private void UpdateTorsoDirection() {
    ...
    if (followGaze == GazeBehavior.Always || firstUpdate) {
      ...
      float angularVelocity = GvrControllerInput.GyroLeft.magnitude;
  ...
  private void UpdateFromController() {
    ...
    Vector3 accel = GvrControllerInput.LeftOrientation * GvrControllerInput.AccelLeft;
  ...
  private void ApplyArmModel() {
    ...
    Quaternion controllerOrientation = GvrControllerInput.LeftOrientation;
    ...
    wristPositionLeft = GvrControllerInput.LeftPosition;
        wristPositionRight = GvrControllerInput.RightPosition;
    ...
    wristRotationLeft = shoulderRotation * controllerOrientation;
        wristRotationRight = GvrControllerInput.RightOrientation;
    ...
    // loganesian -- overwriting wristPosition
