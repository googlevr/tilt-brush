...
public class GvrControllerVisual : GvrBaseControllerVisual
{
...
#if TILT_BRUSH
#if UNITY_EDITOR
        // Returns true if this is a prefab as opposed to a scene object
        if (UnityEditor.AssetDatabase.Contains(this)) {
            // OnValidate() can be called on non-scene objects (ie, prefabs).
            // At least pre-Unity 2018.3, prefabs themselves (as opposed to clones of them)
            // are never present in a scene. Therefore it doesn't make sense to update their
            // visuals. Moreover, OnValidate comes at a weird time which causes Unity to assert
            // when allocating a MaterialPropertyBlock in Initialize()
            return;
        }
#endif
#endif
    ...
    public void UpdateControllerState()
    {
        ...
        displayState.batteryLevel = GvrControllerBatteryLevel.CriticalLow;

        if (controllerID == 0)
        {
            displayState.appButton = GvrControllerInput.AppButtonLeft;
            displayState.clickButton = GvrControllerInput.ClickButtonLeft;
            displayState.touching = GvrControllerInput.IsTouchingLeft;
            displayState.touchPos = GvrControllerInput.TouchPosCenteredLeft;
            displayState.homeButton = GvrControllerInput.HomeButtonStateLeft;
            displayState.batteryCharging = GvrControllerInput.TriggerButtonLeft;
        }
        else
        {
            displayState.appButton = GvrControllerInput.AppButtonRight;
            displayState.clickButton = GvrControllerInput.ClickButtonRight;
            displayState.touching = GvrControllerInput.IsTouchingRight;
            displayState.touchPos = GvrControllerInput.TouchPosCenteredRight;
            displayState.homeButton = GvrControllerInput.HomeButtonStateRight;
            displayState.batteryCharging = GvrControllerInput.TriggerButtonRight;
