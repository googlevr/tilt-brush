...
namespace Gvr.Internal {
  ...
  class AndroidNativeControllerProvider : IControllerProvider {
    ...
    private const int GVR_CONTROLLER_BUTTON_TRIGGER = 6;
    private const int GVR_CONTROLLER_BUTTON_COUNT = 7;
    ...
    private IntPtr[] statePtr_arr = new IntPtr[2];
    ...
    internal AndroidNativeControllerProvider() {
      ...
      for (int i=0; i < 2; i++) {
        statePtr_arr[i] = gvr_controller_state_create();
      }
      ...
      try {
        gvr_controller_state_get_battery_charging(statePtr_arr[0]);
        gvr_controller_state_get_battery_level(statePtr_arr[0]);
    ...
    ~AndroidNativeControllerProvider() {
      ...
      for (int i=0; i < 2; i++) {
        gvr_controller_state_destroy(ref statePtr_arr[i]);
      }
