...
#else
		spectex.rgb = float3(1, 0, 0) * (sin(spectex.r * 2 + scroll*0.5 - IN.uv_SpecTex.x) + 1) * 1;
		spectex.rgb += float3(0, 1, 0) * (sin(spectex.r * 3.3 + scroll * 1 - IN.uv_SpecTex.x) + 1) * 1;
		spectex.rgb += float3(0, 0, 1) * (sin(spectex.r * 4.66 + scroll*0.25 - IN.uv_SpecTex.x) + 1) * 1;
#endif
